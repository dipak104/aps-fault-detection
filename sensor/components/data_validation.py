...
class DataValidation:
    ...
    def drop_missing_values_columns(self, df:pd.DataFrame, report_key_name:str)->Optional[pd.DataFrame]:
        ...
        ======================================================================================
        returns Pandas DataFrame if atleast a single column is available after missing columnsdrop else None
        """
        try:
            ...
            logging.info(f"Columns to be dropped: {drop_column_names}")
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            logging.info(f"Repalce na value in base df")
            ...
            base_df = utils.convert_columns_float(df=base_df, exclude_columns=TARGET_COLUMN)
            train_df = utils.convert_columns_float(df=train_df, exclude_columns=TARGET_COLUMN)
            test_df = utils.convert_columns_float(df=test_df, exclude_columns=TARGET_COLUMN)
            ...
            if train_df_columns_status:
                logging.info(f"As all columns are available in train df: Moving detecting data drift")
                self.data_drift(base_df=base_df, current_df=train_df, report_key_name="data_drift_within_train_dataset")

            if test_df_columns_status:
                logging.info(f"As all columns are available in test df: Moving detecting data drift")
                self.data_drift(base_df=base_df, current_df=test_df, report_key_name="data_drift_within_test_dataset")

            #Write The report
            logging.info("Write report in yaml fie")
            utils.write_yaml_file(file_path=self.data_validation_config.report_file_path, 
                data=self.validation_error)

            data_validation_artifact = artifact_entity.DataValidationArtifact(report_file_path=self.data_validation_config.report_file_path)
            logging.info("Data Validation Artifact")
